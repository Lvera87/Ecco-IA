...
import ApplianceDetailModal from './ui/ApplianceDetailModal';

// Mapeo de imágenes ilustrativas
...
// Mission Redirect Map
const MISSION_REDIRECTS = {
    'Eco-Onboarding': '/profile',
    'Auditor Novato': '/residential-assistant',
};
...
const MissionCard = ({ mission, onClaim, iconMap }) => {
    ...
    const hasRedirect = MISSION_REDIRECTS[title];
    ...
    return (
        <Card className={`p-6 border-2 transition-all cursor-pointer ${statusStyles[status]}`}>
            ...
            <div className="flex items-center justify-between">
                ...
                {status === 'active' && (
                    <Button variant="ghost" size="sm" onClick={() => onClaim(mission, !!mission.related_appliance_type)}>
                        {hasRedirect ? 'Ir Ahora' : mission.related_appliance_type ? 'Ver Equipo' : 'Completar'} <ChevronRight size={16} className="ml-1" />
                    </Button>
                )}
            </div>
        </Card>
...
const MissionsPanel = () => {
    ...
    const handleAction = (mission, isInspect) => {
        ...
        const redirectPath = MISSION_REDIRECTS[mission.title];
        if (redirectPath) {
    ...
    const handleInspectMission = (mission) => {
        ...
        // Logic to find appliance to inspect
    ...
    return (
        <div className="space-y-8 animate-fadeIn">
            {/* Header (optional if parent provides one, but this panel has specific mission header info) */}
            {/* Removed main Title to blend with parent, or we can keep the Stats Bar */}
